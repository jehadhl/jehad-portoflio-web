export function getMusicReactiveDeformState({ elapsed, musicReactiveState }) {
  ...
  const playRoundness =
    musicReactiveState.isPlaying || musicReactiveState.isActive ? 1 : 0;
  ...
  const lowDominance = Math.min(
    1,
    Math.max(0, musicLow - (musicMidSoft * 0.22 + musicHighSoft * 0.32)) * 4.4,
  );
  ...
  const dominantKickHit =
    kickHit * Math.max(0.45, lowDominance, 1 - crowdDensity * 0.26);
  const highTransientMask = Math.min(
    1,
    musicHighSoft * 1.55 + musicMidSoft * 0.34,
  );
  ...
  const highOnlyMask = Math.min(
    1,
    Math.max(0, musicHighSoft - musicMidSoft * 0.72) * 3.4,
  );
  const bassBedImpact =
    Math.min(0.26, Math.max(0, musicLow - 0.3) * 0.82) *
    Math.max(0.22, lowDominance);
  const clapCoreImpact =
    musicMidPulse *
    Math.max(0, midBody - highOnlyMask * 0.28) *
    (1 - crowdDensity * 0.34) *
    0.54;
  const kickCoreImpact =
    dominantKickHit * Math.max(0.42, lowDominance) * (1 - crowdDensity * 0.16);
  ...
  return {
    ...
    blobSpreadMultiplier:
      1 - centerRoundness * 0.46 + soloKeyboard * 0.025 + impact * 0.58,
    surfaceBoost:
      musicMidSoft * 0.045 +
      musicHighSoft * 0.035 +
      soloKeyboard * 0.025 +
    ...
    flowPhase:
      elapsed *
        (musicLow * 0.025 + musicMidSoft * 0.012 + soloKeyboard * 0.01) +
      musicHighSoft * 0.12 +
      impact * 0.22,
