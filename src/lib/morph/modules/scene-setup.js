...
export function createSceneLighting({ scene }) {
  const ambient = new THREE.AmbientLight(
    0xffffff,
    SCENE_CONFIG.ambientIntensity,
  );
  scene.add(ambient);

  const keyLight = new THREE.RectAreaLight(
    0xffffff,
    SCENE_CONFIG.keyIntensity,
    7.5,
    12.5,
  );
...
function createCoreSurfaceTexture() {
  ...
  context.fillText(
    "V 1.1",
    markCellX + cellWidth - 72,
    markCellY + cellHeight - 72,
  );
...
function applyCoreFaceAtlasUvs(geometry) {
  ...
  for (let index = 0; index < position.count; index += 1) {
    ...
    const axis =
      absX >= absY && absX >= absZ ? 0 : absY >= absX && absY >= absZ ? 1 : 2;
    const sign = (axis === 0 ? x : axis === 1 ? y : z) >= 0 ? 1 : -1;
    const face =
      faceCells.find((item) => item.axis === axis && item.sign === sign) ??
      faceCells[0];
...
export function createBlobMeshes({ world }) {
  const glassMaterial = new THREE.MeshPhysicalMaterial({
    color: new THREE.Color("#ffffff"),
    roughness: Math.min(
      SCENE_CONFIG.roughness * 0.18 + SCENE_CONFIG.envBlur * 0.08,
      1,
    ),
    metalness: 0,
