...
export function createScrollPauseState({
  ...
  const updatePauseWrapperState = () => {
    ...
    cachedPauseWrapperEntered =
      rect.top <= window.innerHeight * pauseWrapperEntryRatio;
  ...
  const getWrapperSplitProgress = () => {
    ...
    const splitDistance = Math.max(
      window.innerHeight * (wrapperSplitDistanceVh / 100),
      1,
    );
    return THREE.MathUtils.clamp(
      (window.innerHeight - rect.top) / splitDistance,
      0,
      1,
    );
  };
  ...
  const updatePauseState = () => {
    ...
    cachedShouldPause =
      scrollProgress >= heroStopProgress && pauseWrapperEntered;
