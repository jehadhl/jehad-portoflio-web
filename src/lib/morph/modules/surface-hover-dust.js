...
function dfbm(x, y, time, warp) {
  ...
  const qy = fbm(
    x + 5.2 + pulseC * 0.3,
    y + 1.3 - pulseA * 0.26 + pulseD * 0.1,
    9,
  );
  const rx = fbm(
    x + warp * qx + 1.7 + pulseB * 0.5,
    y + warp * qy + 9.2 + pulseC * 0.44,
    18,
  );
  const ry = fbm(
    x + warp * qx + 8.3 - pulseC * 0.44,
    y + warp * qy + 2.8 + pulseA * 0.5,
    27,
  );
  const base = fbm(
    x + warp * rx + pulseA * 0.42,
    y + warp * ry + pulseB * 0.42,
    36,
  );
...
export function createSurfaceHoverDust({ camera, pointerInput, shellMesh }) {
  ...
  const maskImage = mask.context.createImageData(
    mask.canvas.width,
    mask.canvas.height,
  );
  ...
  const drawMask = (time) => {
    ...
    for (let y = 0; y < height; y += 1) {
      for (let x = 0; x < width; x += 1) {
        ...
        const veining = fbm(
          uvx * 0.66 + 2 + pulseA * 0.22,
          uvy * 0.66 - 3 + pulseB * 0.2,
          48,
        );
        const cleanBreak = fbm(
          uvx * 0.98 + pulseB * 0.2,
          uvy * 0.98 - pulseC * 0.18,
          65,
        );
        ...
        const distance =
          Math.hypot(px - hover.center.x, py - hover.center.y) +
          wobble * 0.045 * hover.radius;
        const reveal =
          1 -
  ...
  const update = ({ scrollProgress = 0, elapsed = 0 }) => {
    const disabled =
      scrollProgress > SCROLL_HIDE_PROGRESS || !pointerInput.isPointerInside();
    let hasHit = false;
