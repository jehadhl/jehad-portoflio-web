...
export function createSurfaceHoverGrid({
  scene,
  camera,
  pointerInput,
  shellMesh,
  ...
  const shouldRevealCell = ({ cell, now }) => {
    ...
    const ringFalloff =
      PULSE_RING_WIDTH * THREE.MathUtils.lerp(1.15, 1.85, pulseProgress);
  ...
  const updateValues = ({
    ...
    const targetMorph = Math.max(
      morphProgress,
      surfaceMorph * (1 - morphProgress * 0.65),
    );
    ...
    const values = [
      ...
      active
        ? `WAVE ${displayedValues.wave.toFixed(2)}`
        : `AUD ${displayedValues.wave.toFixed(2)}`,
    ];
  ...
  const updateLabels = (hit) => {
    ...
    labelTargetWorld
      .copy(hit.point)
      .addScaledVector(labelWorldNormal, SURFACE_LABEL_OFFSET);
    ...
    labels.forEach(({ mesh }, index) => {
      ...
      labelWorld
        ...
        .addScaledVector(
          labelCameraRight,
          (-0.5 + col) * LABEL_SPREAD_X * NET_CELL_SIZE,
        )
        .addScaledVector(
          labelCameraUp,
          (0.5 - row) * LABEL_SPREAD_Y * NET_CELL_SIZE,
        );
  ...
  const update = ({
    ...
    if (
      scrollProgress > LABEL_SCROLL_HIDE_PROGRESS ||
      !pointerInput.isPointerInside()
    ) {
      targetVisualOpacity = 0;
      targetLabelVisualOpacity = 0;
      fadeVisuals();
    ...
    targetLabelVisualOpacity =
      scrollProgress > LABEL_SCROLL_HIDE_PROGRESS ? 0 : 1;
