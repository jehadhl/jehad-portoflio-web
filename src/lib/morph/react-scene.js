...
export async function initMorphScene({ mount, canvas, status }) {
  ...
  const { scene, camera, autoRotateGroup, dragGroup, world } =
    createSceneGraph();
  ...
  const { vertexCount, baseDirections } =
    createBlobGeometryState(blobBaseGeometry);
  ...
  const { dragRotation, dragRotationTarget, pointer, pointerTarget } =
    pointerInput;
  let autoRotationY = 0;
  ...
  const renderFrame = (dt, elapsed) => {
    ...
    const limbImpactEase =
      limbImpact > easedMusicLimbImpact ? 0.22 : limbReleaseEase;
    ...
    const easedCenterRoundness = Math.max(
      0,
      musicReactive.centerRoundness -
        (easedMusicImpact - musicReactive.impact) * 0.42,
    );
    ...
    const coreMassImpact =
      easedMusicReactive.coreMassImpact ?? easedMusicReactive.impact;
    const playingInnerCoreScale =
      musicReactiveState.isPlaying || musicReactiveState.isActive
        ...
        : 1;
    const innerCoreScaleRestore = THREE.MathUtils.smoothstep(
      morphProgress,
      0.12,
      0.32,
    );
    ...
    deformBlob(blobGeometry, elapsed, {
      ...
      reactiveFlowSpeed: THREE.MathUtils.lerp(
        1,
        easedMusicReactive.flowSpeed,
        0.42,
      ),
    ...
    deformBlob(innerGeometry, elapsed, {
      ...
      reactiveSurfaceBoost:
        easedMusicReactive.surfaceBoost * coreSurfaceReaction * coreReturnCalm,
      reactivePointerBoost:
        easedMusicReactive.pointerBoost * 0.8 * coreReturnCalm,
      reactiveFlowPhase:
        easedMusicReactive.flowPhase *
        THREE.MathUtils.lerp(1.1, 0.54, limbReturnDamping),
      reactiveFlowSpeed: THREE.MathUtils.lerp(
        easedMusicReactive.flowSpeed,
        1,
        limbReturnDamping * 0.62,
      ),
      reactiveRippleShift:
        easedMusicReactive.rippleShift *
        THREE.MathUtils.lerp(0.9, 0.28, limbReturnDamping),
