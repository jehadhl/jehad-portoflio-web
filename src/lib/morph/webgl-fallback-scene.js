...
const HDR_ENVIRONMENT_URL = "/scene/studio.hdr";
...
function createHdrEnvironmentController({
  renderer,
  scene,
...
function createBlobAnchorDefinitions() {
  return [
    {
      dir: new THREE.Vector3(1.15, 0.52, 0.08).normalize(),
      pull: 1.35,
      tightness: 6.6,
      speed: 1.1,
      phase: 0,
    ...
    {
      dir: new THREE.Vector3(-1.0, 0.96, 0.34).normalize(),
      pull: 1.05,
      tightness: 7.4,
      speed: 1.32,
      phase: 0.8,
    ...
    {
      dir: new THREE.Vector3(0.98, -0.95, 0.86).normalize(),
      pull: 0.98,
      tightness: 6.9,
      speed: 1.24,
      phase: 1.4,
    ...
    {
      dir: new THREE.Vector3(-1.2, -0.62, -0.22).normalize(),
      pull: 1.18,
      tightness: 7.6,
      speed: 1.08,
      phase: 2.1,
    ...
    {
      dir: new THREE.Vector3(0.18, 1.42, -0.76).normalize(),
      pull: 0.82,
      tightness: 8.4,
      speed: 1.46,
      phase: 2.8,
    ...
    {
      dir: new THREE.Vector3(-0.08, -1.36, -0.84).normalize(),
      pull: 0.9,
      tightness: 8.0,
      speed: 1.18,
      phase: 3.2,
    ...
    {
      dir: new THREE.Vector3(1.02, 0.02, -1.18).normalize(),
      pull: 0.74,
      tightness: 8.8,
      speed: 1.58,
      phase: 3.8,
    ...
    {
      dir: new THREE.Vector3(-0.92, 0.24, 1.1).normalize(),
      pull: 0.7,
      tightness: 9.2,
      speed: 1.66,
      phase: 4.2,
  ...
  return function deformBlob(geometry, time, options = {}) {
    ...
    for (let i = 0; i < vertexCount; i += 1) {
      ...
      for (const anchor of anchorDefinitions) {
        const dot = Math.max(
          0,
          dx * anchor.dir.x + dy * anchor.dir.y + dz * anchor.dir.z,
        );
      ...
      const noise = trigNoise(
        dx,
        dy,
        dz,
        reactiveTime * SCENE_CONFIG.wobbleSpeed,
      );
      const ripple =
        Math.sin(
          (dx + dy) * 8.5 +
      ...
      if (cubeMix > 0) {
        const cubeRadius =
          baseRadius * THREE.MathUtils.lerp(1.0, 0.9, morphProgress);
...
function applySceneTransforms({
  ...
  dragGroup.rotation.y = THREE.MathUtils.lerp(
    dragGroup.rotation.y,
    dragRotation.x,
    0.18,
  );
  dragGroup.rotation.x = THREE.MathUtils.lerp(
  ...
  world.scale.setScalar(
    SCENE_CONFIG.sceneScale *
      responsiveSceneScale *
      THREE.MathUtils.lerp(1, 0.5, morphProgress),
  );
  camera.position.z = THREE.MathUtils.lerp(
    11.5,
    7.4,
    Math.pow(scrollProgress, 1.35),
  );
...
function createFallbackScrollState() {
  ...
  const updateSceneScrollProgress = () => {
    ...
    const total = Math.max(
      heroCanvasTrack.offsetHeight - window.innerHeight,
      1,
    );
...
function createFallbackRenderLoop({
  onFrame,
  onShouldPause,
  onScrollStateChange,
}) {
...
function createFallbackPointerInput({ canvas }) {
  ...
  const updatePointerTargetFromEvent = (event) => {
    const x =
      canvasWidth > 0 ? (event.clientX - canvasLeft) / canvasWidth : 0.5;
    const y =
      canvasHeight > 0 ? (event.clientY - canvasTop) / canvasHeight : 0.5;
  ...
  const onPointerMove = (event) => {
    ...
    dragRotationTarget.y = THREE.MathUtils.clamp(
      dragRotationTarget.y,
      -0.9,
      0.9,
    );
    lastPointerClientX = event.clientX;
...
export async function initWebGLFallbackScene({ mount, canvas, status }) {
  ...
  renderer.setPixelRatio(
    Math.min(window.devicePixelRatio || 1, qualityProfile.pixelRatioCap),
  );
  ...
  const coatMesh = new THREE.Mesh(
    useSafeMobileMaterials ? blobGeometry : coatGeometry,
    coatMaterial,
  );
  ...
  const renderFrame = () => {
    ...
    easedMusicImpact = THREE.MathUtils.lerp(
      ...
      1 -
        Math.pow(
          1 - (musicReactive.impact > easedMusicImpact ? 0.3 : 0.075),
          dt * 60,
        ),
    );
    ...
    easedMusicLimbImpact = THREE.MathUtils.lerp(
      ...
      1 -
        Math.pow(
          1 - (limbImpact > easedMusicLimbImpact ? 0.2 : 0.035),
          dt * 60,
        ),
    );

    const playingInnerCoreScale =
      musicReactiveState.isPlaying || musicReactiveState.isActive
    ...
    const coreSurfaceReaction =
      musicReactiveState.isPlaying || musicReactiveState.isActive
        ? THREE.MathUtils.lerp(0.08, 0.7, easedMusicImpact)
        : 1;
    const coreLobeReaction =
      musicReactiveState.isPlaying || musicReactiveState.isActive
    ...
    deformBlob(blobGeometry, elapsed, {
      ...
      blobSpreadMultiplier: THREE.MathUtils.lerp(
        1,
        musicReactive.blobSpreadMultiplier,
        0.24,
      ),
      morphProgress,
    ...
    deformInnerBlob(innerGeometry, elapsed, {
      ...
      sphericalDirectionBlend:
        musicReactiveState.isPlaying || musicReactiveState.isActive ? 0.68 : 0,
      maxRadius: 1.58,
      forceNormals: !useSafeMobileMaterials && forceAllNormals,
      normalInterval: qualityProfile.shellNormalInterval,
    });
